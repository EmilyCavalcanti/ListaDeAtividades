...
function createListFunctions(localStorageKey, inputId, listId) {
    ...
    function showValues() {
        ...
        for (let i = 0; i < values.length; i++) {
            list.innerHTML += `<li>${values[i] ? values[i]['name'] : 'Nome não definido'}<button id='btn-ok' onclick='removeItem("${values[i] ? values[i]['name'] : ''}", "${localStorageKey}", "${listId}")'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
